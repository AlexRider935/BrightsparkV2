...
async function initializeCollection(collectionName) {
    ...
    if (snapshot.empty) {
        // Add a single placeholder document to create the collection in Firestore.
...
export async function GET() {
    try {
        console.log("Database initialization process started for Layer 2...");

        // Initialize only the 'announcements' collection.
        const announcementStatus = await initializeCollection('announcements');

        console.log("Database initialization process finished.");
        return NextResponse.json({
            message: 'Initialization complete.',
            details: [announcementStatus]
        });
    } catch (error) {
