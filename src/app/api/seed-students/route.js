...
export async function GET() {
    try {
        console.log("Database initialization process started for Layer 1...");

        // Initialize only the 'students' collection as requested.
        const studentStatus = await initializeCollection('students');

        console.log("Database initialization process finished.");
        return NextResponse.json({
            message: 'Initialization complete.',
            details: [studentStatus]
        });
    } catch (error) {
